"""
WebSocket handlers — registers all Socket.IO events.

Events handled:
  connect / disconnect
  subscribe   {session, cols, rows}   → attach PTY, join room
  unsubscribe {session}               → leave room, release PTY ref
  input       {session, data}         → send keystrokes to PTY
  resize      {session, cols, rows}   → resize PTY + tmux window
  scroll      {session, command, lines?} → drive tmux copy-mode
  signal      {session, signal}       → send SIGINT/SIGTSTP etc.
"""
